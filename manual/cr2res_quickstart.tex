...
produces output tables with 2D spectra
...
The spectro-polarimeter splits the light into two beams, and their light passes
through a slit-mask, aka decker, either though the first and third quarter
(2.5'') of the slit, or the second and fourth quarter (cf.~RD01\cite{CIRESMAN}).
These two options make up the two nodding positions, and the recipe
\verb!cr2res_obs_pol! is used in completely analogous way to the nodding-recipe.
...
Then the beams are extracted from each frame, using order traces for each decker
hole. This means many more extractions than for non-polarimetric observations
which is why this recipe takes significantly longer to run.

The resulting spectra are then cleverly combined with each other,
following the demodulation scheme described by \cite{1997MNRAS.291..658D}. This
minimizes instrumental effects and produces the following results, saved in a
FITS table that is analogous to non-polarized spectra:

\begin{itemize}
    \item Intensity spectrum, i.e.~the sum of the beams. Table columns named like \verb!i_INTENS! where \verb!i! is the order number.
    \item The Stokes parameter spectrum, i.e.~V, Q or U, in the columns \verb!i_STOKES!. Which of the Stokes parameters was observed in a dataset should be clear from your data organization, but can also be seen from the header \verb!INS.POL.TYPE!.
    \item The Null spectrum. This is a diagnostic spectrum from the demodulation that characterizes the level of spurious polarization. In the ideal case, its value is very close to zero. \verb!i_NULL!.
