...
\begin{shell}[fontsize=\small]
    ...
    %prompt esorex --output-dir=A1 cr2res_util_extract --slit_frac=sf1,sf2 A.sof
    %prompt esorex --output-dir=A2 cr2res_util_extract --slit_frac=sf3,sf4 A.sof
    %prompt esorex --output-dir=B1 cr2res_util_extract --slit_frac=sf5,sf6 B.sof
    %prompt esorex --output-dir=B2 cr2res_util_extract --slit_frac=sf7,sf8 B.sof 
\end{shell}

The SOF files \verb!A.sof! and \verb!B.sof! contain the calibration files from
the SOF file used to run the nodding recipe in the first step with
the addition of 
\begin{shell}[fontsize=\small]
...
If the user wants a combined spectrum from nodding position A and for each
of the star, they need to combine the A and B spectra - essentially interpolating
one to the wavelength scale of the other. 
